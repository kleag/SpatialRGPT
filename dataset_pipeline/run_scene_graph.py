...
class SceneGraphBuilder:
    def __init__(self, args):
        ...
        self.device = "cuda"
        self.segmenter = SegmentImage(cfg, self.logger, self.device)
        self.reconstructor = PointCloudReconstruction(cfg, self.logger, self.device)
    ...
    def annotate(self, filepath: str) -> SceneGraphNX:
        ...
        progress_file_path = os.path.join(self.cfg.log_folder, f"{filename}.progress")
        if os.path.exists(progress_file_path) and self.cfg.check_exist:
            return {}
        ...
        try:
            ...
            vis_som, detection_list = self.segmenter.process(image_bgr)

            # Lift 2D to 3D, 3D bbox informations are included in detection_list
            detection_list = self.reconstructor.process(
                filename, image_bgr, detection_list
            )
            ...
            detection_list_path = os.path.join(self.cfg.json_folder, f"{filename}.json")
            save_detection_list_to_json(detection_list, detection_list_path)

            relations_3d = self.generator_3d.analyze_relations(detection_list)

            # Save detection list to json
            relations_3d_path = os.path.join(
                self.cfg.json_folder, f"{filename}_rels.json"
            )
            save_relations_3d_to_json(relations_3d, relations_3d_path)

            scene_graph_3d = self.build_scene_graph(detection_list, relations_3d)

            scene_graph_3d_path = os.path.join(
                self.cfg.json_folder, f"{filename}_3dsg.json"
            )
            with open(scene_graph_3d_path, "w") as f:
                f.write(scene_graph_3d.to_json())
            return scene_graph_3d
        except SkipImageException as e:
            # Meet skip image condition
            self.logger.info(f"Skipping processing {filename}: {e}.")
            return SceneGraphNX()
    ...
    def build_scene_graph(self, nodes, relations):
        ...
        nodes_map = {}
        for node in nodes:
            object = self.convert_node(node)
            nodes_map[node["id"]] = object
            graph.add_node(object)
        self.convert_and_add_relations(relations, nodes_map, graph)
        return graph

    def convert_node(
        self, detection: Dict[str, Any], room_id: str = "", robot_id: str = "robot_0"
    ) -> ObjectNode:
        """
        ...
        if obb:
            ...
            ori = self.matrix_to_quaternion(np.asarray(rot_matrix))
    ...
    def convert_and_add_relations(
        self,
        source_relations: List[Tuple[object, object, str, Any]],
    ...
    def matrix_to_quaternion(self, R: list[list[float]]) -> Orientation:
...
def main(args):
    ...
    builder = SceneGraphBuilder(args)
